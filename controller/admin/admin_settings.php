...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($qr_pin !== null && $qr_pin !== '') {
        $query = "UPDATE settings SET time_in = ?, threshold_minute = ?, time_out = ?, qr_pin = ? WHERE id = 1";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("siss", $time_in, $threshold_minute, $time_out, $qr_pin);
    } else {
...
$query = "SELECT time_in, threshold_minute, time_out, qr_pin FROM settings WHERE id = 1";
